namespace :data do
  ...
  task :user_email => :environment do
    ...
    time = Time.new
    file_name = time.month.to_s + time.day.to_s + time.year.to_s + '_user_email.csv'
  ...
  task :weekly_stats => :environment do
    ...
    new_users = User.where('created_at > ?', last_saturday).count
    new_estabs = Establishment.where('created_at > ?', last_saturday).count
